...
for module, cmds in DEFAULT_COMMANDS['commands'].items():
    with open(f'commands/{module}.json', 'w', encoding="utf-8") as f:
        json.dump(cmds, f, indent=2)
    print(f'生成 commands/{module}.json')

# 生成 api_catalog/ 目录
os.makedirs('api_catalog', exist_ok=True)
for api_type, apis in API_CATALOG.items():
    with open(f'api_catalog/{api_type}.json', 'w', encoding="utf-8") as f:
